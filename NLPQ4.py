...
def read_sentences_from_file(file_path):
    with open(file_path in ("C:\Users\jubanen\Documents") = f
         [re.split("\s+", line.rstrip('\n')) in f]
...
if __name__ == '__main__':
    A_dataset = read_sentences_from_file("./sampled.txt")
    A_dataset_test = read_sentences_from_file("./samplet.txt")
    
    A_dataset_model_unsmoothed = BigramLanguageModel(A_dataset)
    A_dataset_model_smoothed = BigramLanguageModel(A_dataset, smoothing=True)

    sorted_vocab_keys = A_dataset_model_unsmoothed.sorted_vocabulary()

    print("---------------- A dataset ---------------\n")
    print("=== UNIGRAM MODEL ===")
    print("- Unsmoothed  -")
    print_unigram_probs(sorted_vocab_keys, A_dataset_model_unsmoothed)
    print("\n- Smoothed  -")
    print_unigram_probs(sorted_vocab_keys, A_dataset_model_smoothed)
    ...
    longest_sentence_len = max([len(" ".join(sentence)) for sentence in A_dataset_test]) + 5
    print("sent", " " * (longest_sentence_len - len("sent") - 2), "uprob\t\tbiprob")
    for sentence in A_dataset_test:
        sentence_string = " ".join(sentence)
        print(sentence_string, end=" " * (longest_sentence_len - len(sentence_string)))
        print("{0:.5f}".format(A_dataset_model_smoothed.calculate_sentence_probability(sentence)), end="\t\t")
        print("{0:.5f}".format(A_dataset_model_smoothed.calculate_bigram_sentence_probability(sentence)))        
        
    print("")

    print("== TEST PERPLEXITY == ")
    print("unigram: ", calculate_unigram_perplexity(A_dataset_model_smoothed, A_dataset_test))
    print("bigram: ", calculate_bigram_perplexity(A_dataset_model_smoothed, A_dataset_test))
    ...
    print("unigram: ", calculate_unigram_perplexity(B_dataset_model_smoothed, B_dataset))
    print("bigram: ", calculate_bigram_perplexity(B_dataset_model_smoothed, B_dataset))

    print("")

    print("PERPLEXITY of test.txt")
    print("unigram: ", calculate_unigram_perplexity(B_dataset_model_smoothed, B_dataset_test))
    print("bigram: ", calculate_bigram_perplexity(B_dataset_model_smoothed, B_dataset_test))
